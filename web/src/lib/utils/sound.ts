// Game sounds — file assets in public/sounds (served at /sounds/*) with the
// old Web Audio synth as offline fallback. Dice roll, double-6, build, jail.
//
// Client-only module: every browser API is guarded so importing this from a
// Server Component never crashes the prerender.
...
export function isMuted(): boolean {
	if (storage()?.getItem(MUTE_KEY) === '1') muted = true;
	return muted;
}
...
export function setMuted(m: boolean): void {
	muted = m;
	try {
		storage()?.setItem(MUTE_KEY, m ? '1' : '0');
	} catch {
		/* ignore */
	}
}
...
// --- File-based sounds (public/sounds/*) ---
